...
HCR2022 <- HCR2022 %>%
  mutate(noise_All_mean = exactextractr::exact_extract(noise_all_masked, HCR2022, 'mean'),
         noise_All_med = exactextractr::exact_extract(noise_all_masked, HCR2022, 'median'),
         noise_day_mean = exactextractr::exact_extract(noise_day_masked, HCR2022, 'mean'),
         noise_day_med = exactextractr::exact_extract(noise_day_masked, HCR2022, 'median'),
         noise_night_mean = exactextractr::exact_extract(noise_night_masked, HCR2022, 'mean'),
         noise_night_med = exactextractr::exact_extract(noise_night_masked, HCR2022, 'median')) %>%
  # suburban thanas
  mutate(drop = ifelse(upazila_th %in% c("Sabujbag", "Mugda", "Shah Ali", "Bhatara", "Khilgaon", "Demra", "Turag", "Uttarkhan", "Badda",
                                         "Khilkhet"), 1, 0),
         # thanas with any suburban areas
         drop_anysub = ifelse(upazila_th %in% c("Pallabi", "Dakkhinkhan", "Jatrabari", "Rupnagar","Sabujbag", "Mugda", "Shah Ali", "Bhatara", "Khilgaon", "Demra", "Turag", "Uttarkhan", "Badda",
                                                "Khilkhet"), 1, 0),
         # thanas with greater than 50% of the area suburban
         drop_50sub = ifelse(upazila_th %in% c("Turag", "Uttarkhan", "Badda", "Khilkhet"), 1, 0),
         HCR_UP_22 = X2022.HCR.U) %>% 
  mutate(catq4 = case_when(HCR_UP_22 >= 0 & HCR_UP_22 < 3.225~ "low",
         ...
         catlmh_d = case_when(HCR_UP_22 >= 0 & HCR_UP_22 < 3.225~ "low",
                              HCR_UP_22 >= 3.225 & HCR_UP_22 < 9.275 ~ "med",
                              HCR_UP_22 >= 9.275 & HCR_UP_22 <= 19.2 ~ "high"),
         catq4 = ordered(catq4, levels = c("low", "med-low", "med-high", "high")),
         catlmh_d = ordered(catlmh_d, levels = c("low", "med", "high")),
         popden_census = Population/total_area) 
